...
#path = "PATH/TO/DATASET/SophiaBeads_64_averaged/SophiaBeads_64_avereaged.xtek"
path_to_dataset  = "/PATH/TO/DATA"

dataset = "SophiaBeads_64_averaged/SophiaBeads_64_averaged.xtekct"

xtek_file = os.path.abspath(os.path.join(path_to_dataset,dataset))

if os.path.exists(xtek_file):
    print("Found file")
else:
    print("Cannot find dataset at ", str(xtek_file), "\nDownload the dataset from https://zenodo.org/record/16474#.XR8JU-hKiUl\nand ensure the path to the dataset is correct")
    sys.exit()

datareader = NikonDataReader(xtek_file=xtek_file, roi=[(1000,1001),(0,2000)])
