...
from ccpi.optimisation.algorithms import PDHG, CGLS, FISTA, SIRT, GradientDescent
...
sirt = SIRT()
sirt.set_up(x_init=x_init, operator=Aop, data=sinogram)
sirt.max_iteration = 1000
sirt.update_objective_interval = 100
sirt.run(1000, verbose = True)

###############################################################################
#
#%%
# Setup and run the Gradient Descent algorithm  
x_init = ig.allocate()  
print("Running Gradient Gescent reconstruction")
f = Norm2Sq(Aop,sinogram)

gd = GradientDescent()
gd.set_up(x_init = x_init, objective_function = f, rate = 1e-5)
gd.max_iteration = 1000
gd.update_objective_interval = 100
gd.run(1000, verbose = True)
...
plt.subplot(3,2,5)
plt.imshow(sirt.get_output().as_array())
plt.colorbar()
...
plt.colorbar()
plt.title('Gradient Descent reconstruction')
...
diff4 = sirt.get_output() - recon_cgls_astra
diff5 = gd.get_output() - recon_cgls_astra
...
plt.subplot(3,3,4)
plt.imshow(diff4.abs().as_array())
plt.title('Diff SIRT vs CGLS astra')
plt.colorbar()

plt.subplot(3,3,5)
plt.imshow(diff5.abs().as_array())
plt.title('Diff Gradient Descent vs CGLS astra')
plt.colorbar()
...
print('Primal Objective (SIRT) {} '.format(sirt.objective[-1]))
print('Primal Objective (Gradient Descent) {} '.format(gd.objective[-1]))
