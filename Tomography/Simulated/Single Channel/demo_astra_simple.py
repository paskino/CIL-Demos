...
from ccpi.optimisation.functions import Norm2Sq, L1Norm, ZeroFunction,\
...
x = Phantom.as_array()
x[int(round(N/4)):int(round(3*N/4)),int(round(N/4)):int(round(3*N/4))] = 0.5
x[int(round(N/8)):int(round(7*N/8)),int(round(3*N/8)):int(round(5*N/8))] = 1
